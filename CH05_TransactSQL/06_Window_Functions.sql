-- NTILE(): Divides ordered rows into a specified number of groups.
-- The groups are as equal in size as possible.

-- Divide all courses into 3 groups based on Top_Id order
select *,
       NTILE(3) over(order by Top_Id asc) as NT -- Divide the rows into 3 groups.
from Course;
...
-- LAST_VALUE() can be surprising with the default window frame.
-- Using ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
-- makes the function consider the entire window.
...
-- The first row has a PERCENT_RANK() of 0.
-- The last row normally has a value of 1.
